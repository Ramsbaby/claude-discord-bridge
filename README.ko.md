...
<img src="https://img.shields.io/badge/크론_태스크-76개-orange?style=flat-square" alt="76 크론">
...
<p>
  <strong>
    ...
    Claude가 자는 동안에도 일하고, 모니터링하고, 학습하게 됩니다. 추가 비용 $0.<br>
    <sub>12 AI 팀 · 76 크론 태스크 · 자가복구 · 로컬 RAG 메모리</sub>
  </strong>
</p>
...
Jarvis는 `claude -p` — Claude Code의 헤드리스 출력 모드 — 위에 씌운 자동화 레이어입니다. Claude를 **Discord**, **예약 크론 작업**, **로컬 RAG 메모리 시스템**에 연결해, 기존 구독을 개인 AI 운영팀으로 바꿔줍니다.

> RAG(Retrieval-Augmented Generation): 과거 대화, 노트, 보고서를 인덱싱해 컨텍스트로 주입하는 방식. 시간이 갈수록 답변이 정교해집니다.
...
| 능동적 자동화 | 76개 예약 태스크 (12팀) | 반응형만 | 비주얼 설정 필요 |
| 자가복구 | 4계층 자동 복구 | ❌ | ❌ |
| 장기 기억 | LanceDB 하이브리드 (로컬) | 드물게 | 선택 플러그인 |
| 컨텍스트 압축 | 98% (Nexus CIG — 아래 설명) | ❌ | ❌ |
...
모든 Discord 답변, 모든 크론 태스크, 모든 AI 팀 보고서가 `claude -p`를 호출합니다. Claude Max 또는 Pro 구독에 포함. API 키 없음, 종량제 없음.
...
**Nexus CIG** (Context Intelligence Gateway) — 모든 툴 출력이 Claude 컨텍스트 창에 들어오기 전에 가로채는 MCP 서버. 실측값: **315 KB → 5.4 KB**. 30분이면 가득 찼을 컨텍스트가 수 시간 지속됩니다.
...
압축 없으면 컨텍스트가 ~30분에 포화됩니다. Nexus CIG 적용 시, 멀티턴 스레드가 auto-compact 트리거 전까지 몇 시간씩 유지됩니다 — 긴 크론 체인에 필수적입니다.
...
```
  ...
  크론 스케줄러  ──►  jarvis-cron.sh  ──►  tasks.json
                                               │
...
| **1 · OS 레벨** | 크래시 발생 시 | `launchd KeepAlive = true` (macOS) / Docker `restart: always` (Linux) — OS 수준 재시작 |
| **2 · Watchdog** | 매 3분 | `watchdog.sh`가 로그 freshness 확인; 스테일 프로세스 종료 및 재시작 |
| **3 · Guardian** | 매 3분 | `launchd-guardian.sh`가 언로드된 LaunchAgent 자동 재등록 |

---

### 페어링 코드 — 접근 제어 (오픈 초대 없음)

Jarvis는 **페어링 코드 시스템**으로 봇과 대화할 수 있는 사람을 제한합니다.
신규 사용자는 `/pair <코드>` 명령어로 일회성 페어링 코드를 입력해야 합니다.
승인된 사용자는 로컬에 저장됩니다 — Discord 역할 해킹도, 공개 초대 링크도 없습니다.
...
- `/run <태스크>`로 76개 크론 태스크 중 아무거나 즉시 실행
- 레이트 리밋 트래커가 일일 상한선을 넘지 않게 관리 (전형적 사용량: ~17%)

**자는 동안에:**
- RAG 인덱스가 매시간 자동 업데이트 *(Obsidian 연동은 선택 사항 — Markdown 폴더 아무거나 됩니다)*
- 팀 보고서, ADR, 일일 결정 사항이 구조화된 Markdown으로 동기화
...
> **사전 요구사항:**
> - **Claude Max 또는 Pro 구독** — 모든 태스크가 `claude -p` 호출 (Pro는 가벼운 사용에 충분; 24/7 운영은 Max 권장)
> - **Claude Code CLI** — `npm install -g @anthropic-ai/claude-code` 후 `claude` 실행하여 브라우저 인증
> - **Node.js 22+** 및 **jq** — `node --version` / `jq --version`으로 확인
> - **Discord 봇 토큰** — [discord.com/developers](https://discord.com/developers/applications)에서 발급; 스크린샷 포함 안내: [discord/SETUP.md](discord/SETUP.md)
> - **플랫폼:** macOS 또는 Linux. Windows 사용자는 WSL2를 사용하세요.
...
실행 확인: `docker compose logs -f` — `Discord connected — Jarvis#XXXX` 메시지가 보여야 합니다.
...
확인 방법: 설정된 Discord 채널에 아무 말이나 입력하면 — 수 초 내로 Jarvis가 응답해야 합니다.

**macOS 24/7 자동재시작 (크래시 시, 로그인 시):**

```bash
launchctl load ~/Library/LaunchAgents/ai.jarvis.discord-bot.plist
launchctl list | grep jarvis   # 서비스가 실행 중으로 표시되어야 합니다
```

> **처음이라면?** [discord/SETUP.md](discord/SETUP.md)에서 봇 생성, 서버/채널 ID 확인, 첫 응답 검증까지 스크린샷과 함께 안내합니다.
...
`depends` 배열이 크로스팀 컨텍스트를 자동 주입합니다: `tqqq-monitor`가 오늘 실행됐다면 그 결과가 스탠드업 프롬프트 앞에 자동으로 붙습니다. 별도 코드 없음.
...
<details>
<summary><strong>Obsidian이 꼭 필요한가요?</strong></summary>

아닙니다. Obsidian은 선택 사항입니다. Jarvis는 어떤 Markdown 폴더도 지식 베이스로 사용할 수 있습니다. vault-sync 기능은 Obsidian과 연동되지만, 봇은 Obsidian 없이도 정상 동작합니다.

</details>

<details>
<summary><strong>Windows에서도 작동하나요?</strong></summary>

네이티브로는 안 됩니다 — Jarvis는 bash 스크립트와 macOS의 `launchd`를 사용합니다. Windows 사용자는 **WSL2** 또는 Docker로 실행하세요.

</details>

<details>
<summary><strong>Claude 레이트 리밋을 다 써버리지 않을까요?</strong></summary>

거의 그렇지 않습니다. 기본 76개 태스크 기준, 하루 사용량은 Claude Max 레이트 리밋의 약 **17%** 수준입니다 — 실제 설치 환경에서 측정한 수치입니다. 내장된 레이트 리밋 트래커가 한도에 가까워지면 태스크를 일시 정지하고 자동으로 재개합니다.

</details>

<details>
<summary><strong>Claude Pro만 있는데 작동하나요?</strong></summary>

네, 가벼운 사용은 됩니다. Claude Pro는 Discord 봇과 소수의 크론 태스크에 충분합니다. 76개 태스크를 24/7 풀 스케줄로 돌리려면 Pro 티어 레이트 리밋을 감안해 Claude Max를 권장합니다.

</details>

<details>
<summary><strong>크론 태스크가 실패하면 어떻게 되나요?</strong></summary>

각 태스크에는 재시도 로직이 내장되어 있습니다 (기본: 지수 백오프, 최대 3회). 그래도 실패하면 Discord 알림과 선택적으로 ntfy 스마트폰 푸시가 전송됩니다. 봇은 계속 실행됩니다 — 한 태스크 실패가 나머지에 영향을 주지 않습니다.
