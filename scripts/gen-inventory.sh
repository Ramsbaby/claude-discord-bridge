...
#
# 출력:
#   ~/Jarvis-Vault/01-system/cron-catalog.md
#   ~/Jarvis-Vault/01-system/launchd-manifest.md
#   ~/Jarvis-Vault/01-system/webhook-registry.md
#
...
OUT_DIR="$HOME/Jarvis-Vault/01-system"
TODAY="$(date +%Y-%m-%d)"
...
"
  ...
  done < <(launchctl list 2>/dev/null | grep -E 'jarvis|openclaw|glances' | awk '{print $1"\t"$2"\t"$3}')
