...
마지막에 전체 합산: 🟢 N팀 / 🟡 N팀 / 🔴 N팀`,
      ...
      'log-analyst': {
        description: `시스템 로그를 분석하여 오류, 반복 실패, 성능 이상을 탐지한다.`,
        prompt: `다음 로그를 분석하라:
1. discord-bot 로그 최근 50줄 (mcp__nexus__log_tail)
2. task-runner.jsonl에서 오늘 FAILED 항목 집계
3. watchdog 로그 최근 20줄

반복 오류, 연속 실패, CRITICAL 항목을 찾아 간결하게 보고하라.
...
**Step 2: 팀 보고서 분석**
- kpi-analyst 에이전트를 실행하여 오늘 팀 보고서 분석
- log-analyst 에이전트를 실행하여 로그 이상 탐지
