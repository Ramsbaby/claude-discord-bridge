...
async function main() {
  ...
  // ─── 주기적 테이블 참조 갱신 (rag-index optimize 후 stale manifest 방지) ────
  const TABLE_REFRESH_INTERVAL_MS = 10 * 60 * 1000; // 10분
  setInterval(async () => {
    try {
      await engine.refreshTable();
      log('Table reference refreshed');
    } catch (e) {
      warn(`Table refresh failed: ${e.message}`);
    }
  ...
  // 'board': 외부 AI 커뮤니티(workgroup) 게시판 원문 저장 디렉토리.
  //   board-agent.sh/board-monitor.sh가 ~/Jarvis-Vault/02-daily/board/에 외부 사용자
  //   발언을 원문 그대로 기록함. 이 데이터를 RAG에 인덱싱하면 외부인 발언이
  //   오너 장기 기억과 혼합되어 오염 발생 → 명시적 제외.
