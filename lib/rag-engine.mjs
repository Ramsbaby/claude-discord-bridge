...
export class RAGEngine {
  ...
  async search(query, limit = 5, opts = {}) {
    ...
    // sourceFilter: 'episodic' → discord-history 소스만 검색
    const sourceFilter = opts.sourceFilter === 'episodic'
      ? "source LIKE '%discord-history%'"
      : null;
    ...
    try {
      ...
      const _applyFilter = (q) => sourceFilter ? q.where(sourceFilter) : q;
      const [raw, normalized] = await Promise.allSettled([
        _applyFilter(this.table.query().fullTextSearch(query, { columns: ['text'] })).limit(limit * 2).toArray(),
        normalizedQuery !== query
          ? _applyFilter(this.table.query().fullTextSearch(normalizedQuery, { columns: ['text'] })).limit(limit * 2).toArray()
          : Promise.resolve([]),
      ]);
    ...
    try {
      ...
      if (sourceFilter) vecQuery = vecQuery.where(sourceFilter);
