...
export class RAGEngine {
  ...
  async indexFile(filePath) {
    ...
    try {
      ...
      // stale 청크 제거: 청크 수가 줄었을 때만 targeted delete 실행 (드문 케이스)
      // 먼저 존재 확인 후 delete → 불필요한 _deletions 엔트리 생성 방지
      const stale = await this.table.query()
        .where(`source = '${safeSource}' AND chunk_index >= ${records.length}`)
        .limit(1).toArray().catch(() => []);
      if (stale.length > 0) {
        await this.table.delete(`source = '${safeSource}' AND chunk_index >= ${records.length}`);
      }
  ...
  async search(query, limit = 5, opts = {}) {
    ...
    // sourceFilter: 'episodic' → discord-history 소스 우선 검색 (boost, 배타 아님)
    const isEpisodic = opts.sourceFilter === 'episodic';
    const episodicFilter = "source LIKE '%discord-history%'";

    // 1. BM25 FTS — primary search, always runs, free (local LanceDB index)
    let bm25Results = [];
    try {
      // episodic 모드: 먼저 discord-history만 검색, 부족하면 전체도 검색
      const _applyFilter = (q, filter) => filter ? q.where(filter) : q;
      const [raw, normalized] = await Promise.allSettled([
        _applyFilter(this.table.query().fullTextSearch(query, { columns: ['text'] }), isEpisodic ? episodicFilter : null).limit(limit * 2).toArray(),
        normalizedQuery !== query
          ? _applyFilter(this.table.query().fullTextSearch(normalizedQuery, { columns: ['text'] }), isEpisodic ? episodicFilter : null).limit(limit * 2).toArray()
          : Promise.resolve([]),
      ]);
      ...
      // episodic 모드에서 결과 부족 시 → 전체 소스 fallback 검색
      if (isEpisodic && bm25Results.length < limit) {
        ...
        const episodicIds = new Set(bm25Results.map(r => r.id));
        // episodic 결과가 앞, fallback이 뒤 → 우선순위 보장
        bm25Results = [...bm25Results, ...fallbackRes.filter(r => !episodicIds.has(r.id))];
      }
    } catch {
    ...
    try {
      ...
      let vecQuery = this.table.search(queryVec).limit(limit * 2);
      if (isEpisodic) vecQuery = vecQuery.where(episodicFilter);
      const vecResults = await vecQuery.toArray();
    ...
    // 3.5 GraphRAG 확장: entity-graph에서 쿼리 관련 엔티티 추출 → 연결 소스 보강
    results = await this._graphExpand(query, results, limit);
  ...
  async _graphExpand(query, results, limit) {
    try {
      ...
      const graphPath = join(process.env.BOT_HOME || join(homedir(), '.jarvis'), 'rag', 'entity-graph.json');
      let graph;
      try {
